                                 ...
                                 )
{
        ...
        try
        {
                ...
                if (ex is TPVNUBEGenNHibernate.Exceptions.ModelException)
                        throw ex;
                throw new TPVNUBEGenNHibernate.Exceptions.DataLayerException ("Error in ClienteCAD.", ex);
        }
...
public int Nuevo (ClienteEN cliente)
{
        try
        {
                SessionInitializeTransaction ();
                if (cliente.Negocio != null) {
                        // Argumento OID y no colección.
                        cliente.Negocio = (TPVNUBEGenNHibernate.EN.Rest.NegocioEN)session.Load (typeof(TPVNUBEGenNHibernate.EN.Rest.NegocioEN), cliente.Negocio.Id);
