...
public IList<LineaComandaEN> GetAllLineaComandaOfPlato (int id)
{
        ...
        try
        {
                ...
                String sql = @"select self FROM LineaComandaEN self inner join self.Plato as target with target.Id=:p_Id";
                IQuery query = session.CreateQuery (sql).SetParameter ("p_Id", id);
